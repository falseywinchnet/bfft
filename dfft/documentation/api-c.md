...
- `bfft_plan_create(n, &plan)` creates a real FFT plan for power-of-two `n >= 4`.
...
All valid BFFT real FFT plans are power-of-two plans, so residue transforms and
residue filters are available for every valid plan.

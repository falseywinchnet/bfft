...
`bruun_dit_kernel.hpp`. Codex moved the scalar merge/expand to
...
## f32 + wide-SIMD status (2026-07-02, files renamed)

Headers renamed: `bruun_kernel.hpp` -> `bruun_dif_kernel.hpp`,
`experimental_radix4_rfft_kernel.hpp` -> `bruun_dit_kernel.hpp`. Class
`DIT_RFFT_kernel`. Full capability matrix + the precise wide-float
(`bruun_v4f`) and AVX2 256-bit designs live in
`notes/dit_capability_checklist.md`. Summary:
- Genuine **f32 scalar** fwd+inv shipped (`*_scalar_f32`, templated `*_t<float>`
  walk over the double twiddle tables); roundtrip ~1e-6.
- **f32 SIMD** entry points (`forward_simd_f32`/`inverse_simd_f32`) exist and are
  correct but route to f32 scalar; the 4-lane `bruun_v4f` block-paired walk is
  the next NEON increment.
- **AVX2 256-bit**: still the companion-model job. Add `bruun_v4`/`V4_*` (4xf64)
  and `bruun_v8f`/`V8F_*` (8xf32) to the abstraction block in
  `bruun_dif_kernel.hpp`, then widen the `*_v_norm` boundaries. No dead stubs.
...
- `src/detail/bruun_dit_kernel.hpp`
  ...
  - Keeps the phase/sincos table helpers from `bruun_dif_kernel.hpp` available for

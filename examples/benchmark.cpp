...
Result bench_one(std::size_t n, int forced_iters, FFTW* fftw, std::mt19937_64& rng) {
  ...
  if (std::isnan(result.fftw_err)) {
    std::printf("err %9s ", "n/a");
  } else {
    std::printf("err %.1e ", result.fftw_err);
  }
...
int main(int argc, char** argv) {
  try {
    ...
    std::printf("%8s %8s %11s %11s %11s %11s %11s %8s %8s  %s\n",
                "N",
                "iters",
                "FFTW_ns",
                "PFFFT_ns",

...
#if defined(__APPLE__)
void split_even_odd(const std::vector<double>& input,
                    std::vector<double>& even,
                    std::vector<double>& odd) {
  const std::size_t n2 = input.size() / 2;
...
void split_even_odd_f32(const std::vector<float>& input,
...
void convert_vdsp_output(const std::vector<double>& real,
                         const std::vector<double>& imag,
                         std::vector<bfft::complex>& output) {
  const std::size_t n2 = real.size();
  output[0].re = 0.5 * real[0];
  output[0].im = 0.0;
  output[n2].re = 0.5 * imag[0];
  output[n2].im = 0.0;
  for (std::size_t k = 1; k < n2; ++k) {
    output[k].re = 0.5 * real[k];
    output[k].im = 0.5 * imag[k];
  }
}

void convert_vdsp_output_f32(const std::vector<float>& real,
                             const std::vector<float>& imag,
                             std::vector<bfft::complex_f32>& output) {
  const std::size_t n2 = real.size();
  output[0].re = 0.5f * real[0];
  output[0].im = 0.0f;
  output[n2].re = 0.5f * imag[0];
  output[n2].im = 0.0f;
  for (std::size_t k = 1; k < n2; ++k) {
    output[k].re = 0.5f * real[k];
    output[k].im = 0.5f * imag[k];
  }
}

double vdsp_output_sample(const std::vector<double>& real,
                          const std::vector<double>& imag,
                          std::size_t k) {
  if (k == real.size()) {
    return 0.5 * imag[0];
...
double vdsp_output_sample_f32(const std::vector<float>& real,
                              const std::vector<float>& imag,
                              std::size_t k) {
  if (k == real.size()) {
    return 0.5 * static_cast<double>(imag[0]);
  }
  return 0.5 * static_cast<double>(real[k]);
}
#endif
...
struct Result {
  ...
  double standard_over_vdsp = NAN;
  double standard_f32_over_vdspf = NAN;
  double standard_over_mkl = NAN;
  double standard_f32_over_mklf = NAN;
  double fftw_err = NAN;
  double fftwf_err = NAN;
  double vdsp_err = NAN;
  double vdspf_err = NAN;
  double mkl_err = NAN;
  double mklf_err = NAN;
...
Result bench_one(std::size_t n, int forced_iters, FFTW* fftw, FFTWf* fftwf, IntelMKL* mkl, std::mt19937_64& rng) {
...
#if defined(__APPLE__)
  FFTSetupD vdsp_plan = nullptr;
  FFTSetup vdspf_plan = nullptr;
  std::vector<double> vdsp_even(n / 2);
  std::vector<double> vdsp_odd(n / 2);
  std::vector<double> vdsp_real(n / 2);
  std::vector<double> vdsp_imag(n / 2);
  std::vector<bfft::complex> vdsp_out(nb);
  std::vector<float> vdspf_even(n / 2);
  std::vector<float> vdspf_odd(n / 2);
  std::vector<float> vdspf_real(n / 2);
  std::vector<float> vdspf_imag(n / 2);
  std::vector<bfft::complex_f32> vdspf_out(nb);
#endif
...
#if defined(__APPLE__)
  const vDSP_Length log2n = static_cast<vDSP_Length>(ilog2_pow2(n));
  vdsp_plan = vDSP_create_fftsetupD(log2n, kFFTRadix2);
  if (vdsp_plan) {
    split_even_odd(original, vdsp_even, vdsp_odd);
    result.vdsp_ns = bench_ns(iters, [&] {
      const std::size_t idx = static_cast<std::size_t>(result.sink) & (n - 1);
      std::copy(vdsp_even.begin(), vdsp_even.end(), vdsp_real.begin());
      std::copy(vdsp_odd.begin(), vdsp_odd.end(), vdsp_imag.begin());
      if ((idx & 1U) == 0) {
        vdsp_real[idx / 2] += 1e-12;
      } else {
        vdsp_imag[idx / 2] += 1e-12;
      }
      DSPDoubleSplitComplex split = { vdsp_real.data(), vdsp_imag.data() };
      vDSP_fft_zripD(vdsp_plan, &split, 1, log2n, FFT_FORWARD);
      const std::size_t k = (static_cast<std::size_t>(result.sink) * 17) % nb;
      result.sink += vdsp_output_sample(vdsp_real, vdsp_imag, k);
    });
    ...
    DSPDoubleSplitComplex split = { vdsp_real.data(), vdsp_imag.data() };
    vDSP_fft_zripD(vdsp_plan, &split, 1, log2n, FFT_FORWARD);
    convert_vdsp_output(vdsp_real, vdsp_imag, vdsp_out);
  }

  vdspf_plan = vDSP_create_fftsetup(log2n, kFFTRadix2);
  if (vdspf_plan) {
    split_even_odd_f32(original_f32, vdspf_even, vdspf_odd);
    result.vdspf_ns = bench_ns(iters, [&] {
      const std::size_t idx = static_cast<std::size_t>(result.sink) & (n - 1);
      std::copy(vdspf_even.begin(), vdspf_even.end(), vdspf_real.begin());
      std::copy(vdspf_odd.begin(), vdspf_odd.end(), vdspf_imag.begin());
      if ((idx & 1U) == 0) {
        vdspf_real[idx / 2] += 1e-7f;
      } else {
        vdspf_imag[idx / 2] += 1e-7f;
      }
      DSPSplitComplex split = { vdspf_real.data(), vdspf_imag.data() };
      vDSP_fft_zrip(vdspf_plan, &split, 1, log2n, FFT_FORWARD);
      const std::size_t k = (static_cast<std::size_t>(result.sink) * 17) % nb;
      result.sink += vdsp_output_sample_f32(vdspf_real, vdspf_imag, k);
    });
    ...
    DSPSplitComplex split = { vdspf_real.data(), vdspf_imag.data() };
    vDSP_fft_zrip(vdspf_plan, &split, 1, log2n, FFT_FORWARD);
    convert_vdsp_output_f32(vdspf_real, vdspf_imag, vdspf_out);
  }
#endif
...
#if defined(__APPLE__)
  if (vdsp_plan) {
    result.vdsp_err = max_abs_complex(standard, vdsp_out);
  }
  if (vdspf_plan) {
    result.vdspf_err = max_abs_complex_f32(standard_f32, vdspf_out);
  }
#endif
...
#if defined(__APPLE__)
  if (vdsp_plan && result.vdsp_ns > 0.0) {
    result.standard_over_vdsp = result.standard_ns / result.vdsp_ns;
  }

  if (vdspf_plan && result.vdspf_ns > 0.0) {
    result.standard_f32_over_vdspf = result.standard_f32_ns / result.vdspf_ns;
  }
#endif
...
#if defined(__APPLE__)
  if (vdsp_plan) {
    vDSP_destroy_fftsetupD(vdsp_plan);
  }
  if (vdspf_plan) {
    vDSP_destroy_fftsetup(vdspf_plan);
  }
#endif
  ...
  print_ns(result.vdsp_ns);
  print_ns(result.vdspf_ns);
  ...
  if (std::isnan(result.standard_over_vdsp)) {
    std::printf("%8s ", "n/a");
  } else {
    std::printf("%8.3f ", result.standard_over_vdsp);
  }

  if (std::isnan(result.standard_f32_over_vdspf)) {
    std::printf("%8s ", "n/a");
  } else {
    std::printf("%8.3f ", result.standard_f32_over_vdspf);
  }
  ...
  if (std::isnan(result.vdsp_err)) {
    std::printf("vdsp64 %9s ", "n/a");
  } else {
    std::printf("vdsp64 %.1e ", result.vdsp_err);
  }

  if (std::isnan(result.vdspf_err)) {
    std::printf("vdsp32 %9s ", "n/a");
  } else {
    std::printf("vdsp32 %.1e ", result.vdspf_err);
  }
...
int main(int argc, char** argv) {
  try {
    ...
    std::printf("BFFT Apple RFFT benchmark. backend: %s, version: %s\n",
...
#if defined(__APPLE__)
    std::printf("vDSP columns use Accelerate vDSP_fft_zrip split real-to-complex setup.\n");
#else
    std::printf("vDSP columns require macOS Accelerate and are unavailable on this platform.\n");
#endif
    ...
    std::printf("%8s %8s %11s %11s %11s %11s %11s %11s %11s %11s %11s %11s %11s %11s %8s %8s %8s %8s %8s %8s %8s %8s  %s\n",
                "N",
                "iters",
                "FFTW_ns",
                "FFTWf_ns",

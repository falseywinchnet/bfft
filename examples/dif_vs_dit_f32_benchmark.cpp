...
// Single-precision counterpart of dif_vs_dit_benchmark.cpp: compares the DIF
// plan's f32 path against DIT_RFFT_kernel::forward_simd_f32 / inverse_simd_f32.
...
std::vector<float> make_signal(std::size_t n) {
    ...
    for (std::size_t i = 0; i < n; ++i) {
        ...
        input[i] = static_cast<float>(std::sin(2.0 * pi * 13.0 * t)
...
double max_abs_complex(const std::vector<ComplexA>& a,
    ...
    for (std::size_t i = 0; i < a.size(); ++i) {
        err = std::max(err, std::abs(static_cast<double>(a[i].re) - static_cast<double>(b[i].re)));
        err = std::max(err, std::abs(static_cast<double>(a[i].im) - static_cast<double>(b[i].im)));
    }
    return err;
}

double max_abs_real(const std::vector<float>& a, const std::vector<float>& b) {
    double err = 0.0;
    for (std::size_t i = 0; i < a.size(); ++i) {
...
result run_one(std::size_t n, int forced_iters) {
    ...
    const std::vector<float> original = make_signal(n);
    std::vector<float> input(original);

    std::vector<bfft::complex_f32> dif_bins(nb);
    std::vector<bruun::complex_f32_t> dit_bins(nb);
    std::vector<bfft::complex_f32> dif_scratch(dif_plan.native_scratch_size());
    std::vector<float> dif_work(dif_plan.work_size_f32());
    std::vector<float> dit_work(static_cast<std::size_t>(dit_plan.work_size_f32()));
    std::vector<float> dif_out(n);
    std::vector<float> dit_out(n);

    dif_plan.forward_f32(original.data(), dif_bins.data(), dif_work.data(), dif_scratch.data());
    dit_plan.forward_simd_f32(original.data(), dit_bins.data(), dit_work.data());
    dif_plan.inverse_f32(dif_bins.data(), dif_out.data());
    dit_plan.inverse_simd_f32(dit_bins.data(), dit_out.data(), dit_work.data());
    ...
    timing t = bench_ns(iters, [&](int i, double sink) {
        input[(static_cast<std::size_t>(i) * 131u + static_cast<std::size_t>(sink)) & (n - 1)] += 1e-4f;
        dif_plan.forward_f32(input.data(), dif_bins.data(), dif_work.data(), dif_scratch.data());
        return static_cast<double>(dif_bins[(static_cast<std::size_t>(i) * 17u) % nb].re);
    });
    ...
    t = bench_ns(iters, [&](int i, double sink) {
        input[(static_cast<std::size_t>(i) * 131u + static_cast<std::size_t>(sink)) & (n - 1)] += 1e-4f;
        dit_plan.forward_simd_f32(input.data(), dit_bins.data(), dit_work.data());
        return static_cast<double>(dit_bins[(static_cast<std::size_t>(i) * 17u) % nb].re);
    });
    ...
    t = bench_ns(iters, [&](int i, double) {
        dif_bins[(static_cast<std::size_t>(i) * 17u) % nb].re += 1e-4f;
        dif_plan.inverse_f32(dif_bins.data(), dif_out.data());
        return static_cast<double>(dif_out[(static_cast<std::size_t>(i) * 31u) & (n - 1)]);
    });
    ...
    dit_plan.forward_simd_f32(original.data(), dit_bins.data(), dit_work.data());
    t = bench_ns(iters, [&](int i, double) {
        dit_bins[(static_cast<std::size_t>(i) * 17u) % nb].re += 1e-4f;
        dit_plan.inverse_simd_f32(dit_bins.data(), dit_out.data(), dit_work.data());
        return static_cast<double>(dit_out[(static_cast<std::size_t>(i) * 31u) & (n - 1)]);
    });
...
int main(int argc, char** argv) {
    try {
        ...
        std::printf("DIF vs experimental DIT RFFT benchmark (f32). backend: %s, DIT backend: %s\n",
                    bfft::backend_name().c_str(),
                    bruun::simd_backend_name());
        std::printf("DIT columns use DIT_RFFT_kernel::forward_simd_f32 / inverse_simd_f32.\n");
        print_header();
        ...
        std::fprintf(stderr, "dif_vs_dit_f32_benchmark failed: %s\n", exc.what());
        return 1;
    }
}
